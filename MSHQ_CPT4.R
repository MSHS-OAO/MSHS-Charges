...
charges <- function(MSH,MSQ){
  ...
  MSHQ <- left_join(MSHQ,rev_map) %>% 
    filter(!is.na(PREMIER)) %>%
    mutate(PARTNER = "729805",
           BUDGET = "0",
           HOSP = "NY0014",
           START = as.Date(paste0(MONTH,"/1/",Year),format = "%m/%d/%Y"))
  MSHQ <- mutate(MSHQ,
                 END = paste0(MONTH,"/",days_in_month(MONTH),"/",Year),
                 START = paste0(substr(START,6,7),"/",substr(START,9,10),"/",Year)) %>%
    group_by(PARTNER,HOSP,ORACLE.CC,CPT,START,END,CPT,BUDGET,CPT.GROUP) %>%
    summarise(QTY = sum(QTY, na.rm=T)) %>%
    select(PARTNER,HOSP,ORACLE.CC,CPT,START,END,CPT,QTY,BUDGET,CPT.GROUP)
  MSHQ <<- MSHQ
}
...
master <- function(Q){
  ...
  master_trend <- master %>%
    mutate(`Concatenate for lookup` = paste0(Year,"Q",Q,CPT)) %>%
    left_join(.,cpt_ref) %>%
    ...
    filter(LABOR > 0) %>%
    group_by(ORACLE.CC,END)%>%
    summarise(LABOR = sum(LABOR,na.rm = T))%>%
    pivot_wider(id_cols = ORACLE.CC,names_from = END,values_from = LABOR)
  master <<- master
